...
import com.shinow.entity.TMeMerchandiseCInfoEntity;
 ...
 */
public class ProstatusinfoWorkAction extends BaseAction {

    @Resource
    private BaseDAO<TMeProStatusInfoEntity> proDao;

    private List<TMeProStatusInfoEntity> proList;

    private TMeProStatusInfoEntity pro;
    ...
    public String qureypro(){
        countNumed=proDao.queryRecordCount("select count(*) from TMeProStatusInfoEntity");
        proList = proDao.queryForPage("from TMeProStatusInfoEntity",page,limit);
        success ="true";
        return SUCCESS;
    }
...
//            defaultVauel(mccI);
            try {
                Serializable o= proDao.insert(pro);
                if(null==o){
                    success="false";
                    message="插入失败请重试！";
                }else {
    ...
    public String updatapro(){
        boolean coad = proDao.update(pro);
        if(coad){
            success="true";
            message="成功";
        }else{
    ...
    public String delect(){
        boolean coad = proDao.delete(pro);
        if(coad){
            success="true";
            message="成功";
        }else{
    ...
    public List<TMeProStatusInfoEntity> getProList() {
        return proList;
    }

    public void setProList(List<TMeProStatusInfoEntity> proList) {
        this.proList = proList;
    }

    public TMeProStatusInfoEntity getPro() {
        return pro;
    }

    public void setPro(TMeProStatusInfoEntity pro) {
        this.pro = pro;
    }
}
