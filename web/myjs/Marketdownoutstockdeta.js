 ...
 */
Ext.define('myjs.Marketdownoutstockdeta', {
    ...
    initComponent: function () {
        ...
        var dajax = Ext.Ajax.request({
            url: '/queryField?tableName=tme_outstockinfo',
            reader: 'json',
            root: 'fElist',
            async: false,
...
//            }
    ...
    myLook: function () {
        var record = Ext.getCmp('CXoutstockgrid').getSelectionModel().getSelection()[0];
        if(record){
        var defaultvalue=record.raw;
        if(!Ext.getCmp('mainoutstockwindow')){
            Ext.require('myjs.CXdownwindowpanel',function(){
                 Ext.create('Ext.window.Window',{
                     title:  '出库详细信息',
                     id:"mainoutstockwindow",
                     layout:'fit',
                     height: 800,
                     width: 1200,
                    items:[Ext.create('myjs.CXdownwindowpanel',{defaultvalue:defaultvalue})]
                }).show().center()
            });
        }
        }else{
        ...
        if (record) {
            Ext.MessageBox.show({
                ...
                fn: function (btn) {
                    if (btn === 'yes') {
                        Ext.Ajax.request({
                            url: '/outStockInfoWorkdelect?outstock.outbillcode=' + record.get('outbillcode'),
                            success: function (action) {
                                ...
                                if (result.success) {
                                    ...
                                    Ext.getCmp('CXoutstockgrid').store.reload();
                                }
                            }
                        });
                    }
